...
class RouteGraceTrackerTest {
    ...
    @Test
    fun wrongRouteIsImmediateAndDoesNotPoisonNextGraceWindow() {
        assertEquals(
            RouteGraceTracker.Observation.WRONG_DEVICE,
            tracker.observe(1L, routedToSpeaker = false, routedToOtherDevice = true),
        )
        assertEquals(
            RouteGraceTracker.Observation.GRACE,
            tracker.observe(2_000_000_001L, routedToSpeaker = false, routedToOtherDevice = false),
        )
    }
}
